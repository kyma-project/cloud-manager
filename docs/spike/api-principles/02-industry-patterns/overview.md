...
| [5](./05-typed-provider-sub-struct.md) | Typed provider sub-struct | Crossplane `forProvider`, CAPI infrastructure contract | KCP NfsInstance sub-struct alignment |
| [6](./06-unstructured-payload.md) | Portable container + unstructured payload | StorageClass `parameters`, k8s `x-kubernetes-preserve-unknown-fields` | Future WafPolicy rules |
