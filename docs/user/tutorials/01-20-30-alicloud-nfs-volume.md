# Using NFS in Alibaba Cloud

> [!WARNING]
> This is a beta feature available only per request for SAP-internal teams.

This tutorial explains how to use Network File System (NFS) to create ReadWriteMany (RWX) volumes in Alibaba Cloud. You can use the created RWX volume from multiple workloads.
...
2. Create an AlicloudNfsVolume resource.
...
3. Wait for the AlicloudNfsVolume to be in the `Ready` state.

   ```shell
   kubectl -n $NAMESPACE wait --for=condition=Ready alicloudnfsvolume/my-vol --timeout=300s
   ```

   Once the newly created AlicloudNfsVolume is provisioned, you should see the following message:

   ```console
   alicloudnfsvolume.cloud-resources.kyma-project.io/my-vol condition met
   ```

4. Observe the generated PersistentVolume:

   ```shell
   export PV_NAME=`kubectl get alicloudnfsvolume my-vol -n $NAMESPACE -o jsonpath='{.status.id}'`
...
6. Create two workloads that both write to the volume:
   ...
   kind: Deployment
   metadata:
     name: alicloudnfsvolume-demo
   spec:
     selector:
       matchLabels:
         app: alicloudnfsvolume-demo
     replicas: 2
     template:
       metadata:
         labels:
...
7. Print the logs of one of the workloads, run:
   ...
   ```shell
   kubectl logs -n $NAMESPACE `kubectl get pod -n $NAMESPACE -l app=alicloudnfsvolume-demo -o=jsonpath='{.items[0].metadata.name}'`
   ```
   ...
   ```console
   ...
   Hello from alicloudnfsvolume-demo-869c89df4c-dsw97: 19
   Hello from alicloudnfsvolume-demo-869c89df4c-dsw97: 20
   File content:
   Hello from alicloudnfsvolume-demo-869c89df4c-8z9zl: 20
   Hello from alicloudnfsvolume-demo-869c89df4c-l8hrb: 1
   Hello from alicloudnfsvolume-demo-869c89df4c-dsw97: 1
   Hello from alicloudnfsvolume-demo-869c89df4c-l8hrb: 2
   Hello from alicloudnfsvolume-demo-869c89df4c-dsw97: 2
   Hello from alicloudnfsvolume-demo-869c89df4c-l8hrb: 3
   ...
   ```
...
1. Remove the created workloads:
   ...
   ```shell
   kubectl delete -n $NAMESPACE deployment alicloudnfsvolume-demo
   ```
...
3. Remove the created AlicloudNfsVolume:

   ```shell
   kubectl delete -n $NAMESPACE alicloudnfsvolume my-vol
   ```
