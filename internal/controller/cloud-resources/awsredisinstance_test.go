...
var _ = Describe("Feature: SKR AwsRedisInstance", func() {
	...
	It("Scenario: SKR AwsRedisInstance authSecret is modified", func() {
		awsRedisInstanceName := "auth-secret-modified-aws-instance"
		...
		By("When AwsRedisInstance authSecret config is modified with new labels, annotations, and extraData", func() {
			...
			Eventually(UpdateAwsRedisInstance).
				WithArguments(
					infra.Ctx(), infra.SKR().Client(), awsRedisInstance,
					WithAwsRedisInstanceAuthSecretExtraData(newExtraData),
				).
				Should(Succeed())
		})
		...
		By("Then SKR auth Secret is updated with new labels", func() {
			Eventually(func() map[string]string {
				secretKey := types.NamespacedName{Name: authSecretName, Namespace: awsRedisInstance.Namespace}
