...
pkg/kcp/provider/alicloud/mock/redisStore.go                full file — unified instance + cluster store methods
pkg/kcp/provider/alicloud/mock/redisClientViews.go          full file — unified instance + cluster client views
...
internal/controller/cloud-control/suite_test.go             instance + cluster mock wiring (cluster provider benign until PR-C)
...
internal/controller/cloud-resources/suite_test.go           instance controller registration (one line)
internal/api-tests/skr_alicloudredisinstance_test.go
pkg/skr/runtime/looper/installer_test.go                    alicloudredisinstance CRD entry (one line)
cmd/main.go                                                 SKR AlicloudRedisInstance controller
```

**Note:** `pkg/testinfra/dsl/alicloudRedisInstance.go`, `commonIpRange.go`, and `kcpIpRange.go`
are assigned to PR-A (full file with all 4 resource helpers). PR-B uses them as-is.
`pkg/testinfra/run.go` is also in PR-A (mock init only) — no changes needed in PR-B.
...
pkg/kcp/rediscluster/reconciler.go                          new file — wire alicloud into KCP dispatcher
internal/controller/cloud-control/rediscluster_controller.go
internal/controller/cloud-control/rediscluster_alicloud_test.go
internal/controller/cloud-control/suite_test.go             cluster controller wiring (one line)
cmd/main.go                                                 KCP RedisCluster provider registration
```

**Note on mock:** `redisStore.go` and `redisClientViews.go` already contain full cluster support
from PR-A (the mock is architecturally unified). PR-C's diff against those files is zero.
...
internal/controller/cloud-resources/suite_test.go           cluster controller registration (one line)
internal/api-tests/skr_alicloudrediscluster_test.go
pkg/skr/runtime/looper/installer_test.go                    alicloudrediscluster CRD entry (one line)
...
| `mock/` (5 files) | Create (full — unified instance + cluster) | — | Zero-delta | — |
| `pkg/testinfra/dsl/alicloudRedisInstance.go` | Create (full — all 4 helpers) | Use | Use | Use |
| `suite_test.go` KCP | Instance + cluster mock wiring | — | Cluster controller wiring | — |
| `suite_test.go` SKR | — | Instance registration | — | Cluster registration |
| `pkg/skr/iprange/reconciler.go` | — | Instance guard (one line) | — | Cluster guard (one line) |
| `installer_test.go` | — | Instance CRD entry | — | Cluster CRD entry |
| `cmd/main.go` | KCP instance provider | SKR instance | KCP cluster provider | SKR cluster |
| `pkg/testinfra/run.go` | Mock init only | — | — | — |
