...
func stopIfVolumeNotReady(ctx context.Context, st composed.State) (error, context.Context) {
	...
	return composed.PatchStatus(restore).
		...
		ErrorLogMessage("Failed updating AwsNfsVolumeRestore error status with NfsVolumeNotReady condition").
		SuccessLogMsg("Forgetting AwsNfsVolumeRestore with NfsVolume not ready").
