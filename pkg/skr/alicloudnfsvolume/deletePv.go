...
func deletePv(ctx context.Context, st composed.State) (error, context.Context) {
	...
	state.ObjAsAlicloudNfsVolume().Status.State = cloudresourcesv1beta1.StateDeleting
	err, _ := composed.UpdateStatus(state.ObjAsAlicloudNfsVolume()).
		...
		ErrorLogMessage("Error setting ConditionReasonDeletingPV condition on AlicloudNfsVolume").
		SuccessErrorNil().
	...
	logger.Info("Deleting PV for AlicloudNfsVolume")

	err = state.Cluster().K8sClient().Delete(ctx, state.Volume)
	if err != nil {
		return composed.LogErrorAndReturn(err, "Error deleting PV for AlicloudNfsVolume", composed.StopWithRequeue, ctx)
	}
