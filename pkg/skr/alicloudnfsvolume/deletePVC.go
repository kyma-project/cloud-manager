...
func deletePVC(ctx context.Context, st composed.State) (error, context.Context) {
	...
	err, _ := composed.UpdateStatus(state.ObjAsAlicloudNfsVolume()).
		...
		ErrorLogMessage("Error setting ConditionReasonDeletingPVC condition on AlicloudNfsVolume").
		SuccessErrorNil().
	...
	logger.Info("Deleting PVC for AlicloudNfsVolume")

	err = state.Cluster().K8sClient().Delete(ctx, state.PVC)
	if err != nil {
		return composed.LogErrorAndReturn(err, "Error deleting PVC for AlicloudNfsVolume", composed.StopWithRequeue, ctx)
	}
