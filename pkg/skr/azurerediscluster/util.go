...
func getAuthSecretName(state *State) string {
	...
	return state.ObjAsAzureRedisCluster().Name
}
...
func getAuthSecretLabels(state *State) map[string]string {
	...
	azureRedis := state.ObjAsAzureRedisCluster()
	labelsBuilder.WithCustomLabel(cloudresourcesv1beta1.LabelRedisClusterStatusId, azureRedis.Status.Id)
	labelsBuilder.WithCustomLabel(cloudresourcesv1beta1.LabelRedisClusterNamespace, azureRedis.Namespace)
...
func RedisTierToSKUCapacityConverter(redisTier cloudresourcesv1beta1.AzureRedisClusterTier) (int, error) {
	azureRedisSKUCapacity, exists := azureRedisTierToAzureRedisSKUCapacityValueMap[redisTier]

	if !exists {
		return 0, errors.New("unknown azure cluster redis tier")
	}

	return azureRedisSKUCapacity, nil
}
