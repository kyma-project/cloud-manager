...
func pvcRemoveFinalizer(ctx context.Context, st composed.State) (error, context.Context) {
	...
	if err != nil {
		return composed.LogErrorAndReturn(err, "Error removing finalizer from CceeNfsVolume PVC", composed.StopWithRequeue, ctx)
	}
	if removed {
		logger.Info("Removed CceeNfsVolume PVC finalizer")
	}
