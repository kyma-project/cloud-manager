...
func deleteAuthSecret(ctx context.Context, st composed.State) (error, context.Context) {
	...
	err, _ := composed.UpdateStatus(state.ObjAsAwsRedisCluster()).
		...
		ErrorLogMessage("Error setting ConditionReasonDeletingAuthSecret condition on AwsRedisCluster").
		SuccessErrorNil().
	...
	logger.Info("Deleting AuthSecret for AwsRedisCluster")

	err = state.Cluster().K8sClient().Delete(ctx, state.AuthSecret)
	if err != nil {
		return composed.LogErrorAndReturn(err, "Error deleting AuthSecret for AwsRedisCluster", composed.StopWithRequeue, ctx)
	}
