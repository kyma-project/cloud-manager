...
func addFinalizer(ctx context.Context, st composed.State) (error, context.Context) {
	...
	if err != nil {
		return composed.LogErrorAndReturn(err, "Error updating GcpVpcPeering after adding the finalizer", composed.StopWithRequeue, ctx)
	}

	logger.Info("Finalizer added to SKR GcpVpcPeering, requeue")
