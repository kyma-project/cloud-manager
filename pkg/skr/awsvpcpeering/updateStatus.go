...
func updateStatus(ctx context.Context, st composed.State) (error, context.Context) {
	...
	skrCondErr := meta.FindStatusCondition(state.ObjAsAwsVpcPeering().Status.Conditions, cloudresourcesv1beta1.ConditionTypeError)
	...
	if kcpCondErr != nil && (skrCondErr == nil || skrCondErr.Message != kcpCondErr.Message) {
		...
		return composed.UpdateStatus(obj).
			...
			ErrorLogMessage("Error updating SKR AwsVpcPeering status with not ready condition due to KCP error").
			SuccessLogMsg("Updated and forgot SKR AwsVpcPeering status with Error condition").
	...
	if kcpCondReady != nil && skrCondReady == nil {
		...
		return composed.UpdateStatus(obj).
			...
			ErrorLogMessage("Error updating SKR AwsVpcPeering status with ready condition").
