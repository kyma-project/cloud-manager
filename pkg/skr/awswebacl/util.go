...
// convertStatement1 converts a Statement1 (nesting level 1) to AWS WAF format
// Can contain: logical operators (AND/OR/NOT) + leaf statements (NO RateBased, NO ManagedRuleGroup)
func convertStatement1(stmt cloudresourcesv1beta1.AwsWebAclStatement1) (*wafv2types.Statement, error) {
	statement := &wafv2types.Statement{}
	count := 0

	// Handle nested NOT statement
	if stmt.NotStatement != nil {
		notStmt, err := convertNotStatement1(stmt.NotStatement)
		if err != nil {
			return nil, err
		}
		statement.NotStatement = notStmt
...
// convertStatement2 converts a Statement2 (nesting level 2) to AWS WAF format
// Can contain: logical operators (AND/OR/NOT) + leaf statements
func convertStatement2(stmt cloudresourcesv1beta1.AwsWebAclStatement2) (*wafv2types.Statement, error) {
	statement := &wafv2types.Statement{}
	count := 0

	// Handle logical operators
	if stmt.AndStatement != nil {
		andStmt, err := convertAndStatement2(stmt.AndStatement)
		if err != nil {
			return nil, err
		}
		statement.AndStatement = andStmt
	...
	if stmt.OrStatement != nil {
		orStmt, err := convertOrStatement2(stmt.OrStatement)
		if err != nil {
			return nil, err
		}
		statement.OrStatement = orStmt
	...
	if stmt.NotStatement != nil {
		notStmt, err := convertNotStatement2(stmt.NotStatement)
		if err != nil {
			return nil, err
		}
		statement.NotStatement = notStmt
	...
	if count == 0 {
		return nil, fmt.Errorf("statement2 must have exactly one condition set")
	}
	if count > 1 {
		return nil, fmt.Errorf("statement2 must have exactly one condition set, found %d", count)
	}

	return statement, nil
}

// convertStatement3 converts a Statement3 (nesting level 3) to AWS WAF format
// Can contain: logical operators (AND/OR/NOT) + leaf statements
func convertStatement3(stmt cloudresourcesv1beta1.AwsWebAclStatement3) (*wafv2types.Statement, error) {
	statement := &wafv2types.Statement{}
	count := 0

	// Handle logical operators
	if stmt.AndStatement != nil {
		andStmt, err := convertAndStatement3(stmt.AndStatement)
		if err != nil {
			return nil, err
		}
		statement.AndStatement = andStmt
	...
	if stmt.OrStatement != nil {
		orStmt, err := convertOrStatement3(stmt.OrStatement)
		if err != nil {
			return nil, err
		}
		statement.OrStatement = orStmt
	...
	if stmt.NotStatement != nil {
		notStmt, err := convertNotStatement3(stmt.NotStatement)
		if err != nil {
			return nil, err
		}
		statement.NotStatement = notStmt
	...
	if count == 0 {
		return nil, fmt.Errorf("statement3 must have exactly one condition set")
	}
	if count > 1 {
		return nil, fmt.Errorf("statement3 must have exactly one condition set, found %d", count)
	}

	return statement, nil
}

// convertStatement4 converts a Statement4 (nesting level 4 - leaf only) to AWS WAF format
// Can contain: ONLY leaf statements (NO logical operators)
func convertStatement4(stmt cloudresourcesv1beta1.AwsWebAclStatement4) (*wafv2types.Statement, error) {
	statement := &wafv2types.Statement{}
	count := 0
	...
	if count == 0 {
		return nil, fmt.Errorf("statement4 must have exactly one condition set")
	}
	if count > 1 {
		return nil, fmt.Errorf("statement4 must have exactly one condition set, found %d", count)
	}
...
// ===== Level 1 Operator Converters =====

func convertAndStatement1(and *cloudresourcesv1beta1.AwsWebAclAndStatement1) (*wafv2types.AndStatement, error) {
	if and == nil || len(and.Statements) < 2 {
		return nil, fmt.Errorf("and statement1 requires at least 2 nested statements")
	}

	statements := make([]wafv2types.Statement, 0, len(and.Statements))
	for i, stmt := range and.Statements {
		converted, err := convertStatement2(stmt)
		if err != nil {
			return nil, fmt.Errorf("error converting and statement1[%d]: %w", i, err)
		}
		statements = append(statements, *converted)
	}
...
func convertOrStatement1(or *cloudresourcesv1beta1.AwsWebAclOrStatement1) (*wafv2types.OrStatement, error) {
	if or == nil || len(or.Statements) < 2 {
		return nil, fmt.Errorf("or statement1 requires at least 2 nested statements")
	}
	...
	for i, stmt := range or.Statements {
		converted, err := convertStatement2(stmt)
		if err != nil {
			return nil, fmt.Errorf("error converting or statement1[%d]: %w", i, err)
		}
		statements = append(statements, *converted)
	}
...
func convertNotStatement1(not *cloudresourcesv1beta1.AwsWebAclNotStatement1) (*wafv2types.NotStatement, error) {
	if not == nil {
		return nil, fmt.Errorf("not statement1 cannot be nil")
	}

	converted, err := convertStatement2(not.Statement)
	if err != nil {
		return nil, fmt.Errorf("error converting not statement1: %w", err)
	}
...
// ===== Level 2 Operator Converters =====

func convertAndStatement2(and *cloudresourcesv1beta1.AwsWebAclAndStatement2) (*wafv2types.AndStatement, error) {
	if and == nil || len(and.Statements) < 2 {
		return nil, fmt.Errorf("and statement2 requires at least 2 nested statements")
	}

	statements := make([]wafv2types.Statement, 0, len(and.Statements))
	for i, stmt := range and.Statements {
		converted, err := convertStatement3(stmt)
		if err != nil {
			return nil, fmt.Errorf("error converting and statement2[%d]: %w", i, err)
		}
		statements = append(statements, *converted)
	}
...
func convertOrStatement2(or *cloudresourcesv1beta1.AwsWebAclOrStatement2) (*wafv2types.OrStatement, error) {
	if or == nil || len(or.Statements) < 2 {
		return nil, fmt.Errorf("or statement2 requires at least 2 nested statements")
	}
	...
	for i, stmt := range or.Statements {
		converted, err := convertStatement3(stmt)
		if err != nil {
			return nil, fmt.Errorf("error converting or statement2[%d]: %w", i, err)
		}
		statements = append(statements, *converted)
	}
...
func convertNotStatement2(not *cloudresourcesv1beta1.AwsWebAclNotStatement2) (*wafv2types.NotStatement, error) {
	if not == nil {
		return nil, fmt.Errorf("not statement2 cannot be nil")
	}

	converted, err := convertStatement3(not.Statement)
	if err != nil {
		return nil, fmt.Errorf("error converting not statement2: %w", err)
	}
...
// ===== Level 3 Operator Converters =====

func convertAndStatement3(and *cloudresourcesv1beta1.AwsWebAclAndStatement3) (*wafv2types.AndStatement, error) {
	if and == nil || len(and.Statements) < 2 {
		return nil, fmt.Errorf("and statement3 requires at least 2 nested statements")
	}

	statements := make([]wafv2types.Statement, 0, len(and.Statements))
	for i, stmt := range and.Statements {
		converted, err := convertStatement4(stmt)
		if err != nil {
			return nil, fmt.Errorf("error converting and statement3[%d]: %w", i, err)
		}
		statements = append(statements, *converted)
	}
...
func convertOrStatement3(or *cloudresourcesv1beta1.AwsWebAclOrStatement3) (*wafv2types.OrStatement, error) {
	if or == nil || len(or.Statements) < 2 {
		return nil, fmt.Errorf("or statement3 requires at least 2 nested statements")
	}
	...
	for i, stmt := range or.Statements {
		converted, err := convertStatement4(stmt)
		if err != nil {
			return nil, fmt.Errorf("error converting or statement3[%d]: %w", i, err)
		}
		statements = append(statements, *converted)
	}
...
func convertNotStatement3(not *cloudresourcesv1beta1.AwsWebAclNotStatement3) (*wafv2types.NotStatement, error) {
	if not == nil {
		return nil, fmt.Errorf("not statement3 cannot be nil")
	}

	converted, err := convertStatement4(not.Statement)
	if err != nil {
		return nil, fmt.Errorf("error converting not statement3: %w", err)
	}
