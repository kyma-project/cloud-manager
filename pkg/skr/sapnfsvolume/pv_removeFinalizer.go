...
func pvRemoveFinalizer(ctx context.Context, st composed.State) (error, context.Context) {
	...
	if err != nil {
		return composed.LogErrorAndReturn(err, "Error removing finalizer from SapNfsVolume PV", composed.StopWithRequeue, ctx)
	}
	if removed {
		logger.Info("Removed SapNfsVolume PV finalizer")
	}
