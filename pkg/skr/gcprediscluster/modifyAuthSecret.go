...
func modifyAuthSecret(ctx context.Context, st composed.State) (error, context.Context) {
	...
	desiredLabels := getAuthSecretLabels(state.ObjAsGcpRedisCluster())
	desiredAnnotations := getAuthSecretAnnotations(state.ObjAsGcpRedisCluster())
	...
	if err != nil {
		return composed.LogErrorAndReturn(err, "Error updating secret for GcpRedisCluster", composed.StopWithRequeue, ctx)
	}

	logger.Info("AuthSecret for GcpRedisCluster updated")
