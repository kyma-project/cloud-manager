...
`Notify`, `recordNotifConnect` — with a dummy sleeping handler instead of a real per-SKR manager (no
IO, no real 10s connect). It seeds a large fleet, designates a few "hot" SKRs that fire notifications
constantly, drives a skewed notification stream, and measures per-SKR reconnect gaps.
...
Use it to validate any change to the looper's queueing/fairness logic. It reproduces the production
symptom (a fixed set of SKRs starved with >20-min gaps while hot SKRs stay fresh) and asserts it is
gone.
