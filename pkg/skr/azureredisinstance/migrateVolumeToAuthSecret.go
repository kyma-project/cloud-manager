...
func migrateVolumeToAuthSecret(ctx context.Context, st composed.State) (error, context.Context) {
	...
	// If both fields are set, log a warning and prefer AuthSecret
	if obj.Spec.Volume != nil && obj.Spec.AuthSecret != nil {
		...
		if err != nil {
			return composed.LogErrorAndReturn(err, "Error updating AzureRedisInstance after clearing deprecated 'volume' field", composed.StopWithRequeue, ctx)
		}
		return nil, ctx
	}
