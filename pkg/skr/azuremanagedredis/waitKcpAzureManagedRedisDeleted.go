...
func waitKcpAzureManagedRedisDeleted(ctx context.Context, st composed.State) (error, context.Context) {
	state := st.(*State)
	...
	if state.KcpAzureManagedRedis == nil {
		logger.Info("KCP AzureManagedRedis is deleted")
		return nil, ctx
	}
	...
	logger.Info("Waiting for KCP AzureManagedRedis to be deleted")
