...
func deleteAuthSecret(ctx context.Context, st composed.State) (error, context.Context) {
	...
	err, _ := composed.UpdateStatus(amr).
		...
		ErrorLogMessage("Error setting ConditionReasonDeletingAuthSecret condition on AzureManagedRedis").
		SuccessErrorNil().
	...
	logger.Info("Deleting AuthSecret for AzureManagedRedis")

	err = state.Cluster().K8sClient().Delete(ctx, state.AuthSecret)
	if err != nil {
		return composed.LogErrorAndReturn(err, "Error deleting AuthSecret for AzureManagedRedis", composed.StopWithRequeue, ctx)
	}
