...
func modifyAuthSecret(ctx context.Context, st composed.State) (error, context.Context) {
	...
	if state.AuthSecret == nil {
		logger.Info("can't modify auth secret, not found")
		return nil, ctx
	}
	...
	desiredLabels := getAuthSecretLabels(state.ObjAsAzureManagedRedis())
	desiredAnnotations := getAuthSecretAnnotations(state.ObjAsAzureManagedRedis())
	...
	if err != nil {
		return composed.LogErrorAndReturn(err, "Error updating secret for AzureManagedRedis", composed.StopWithRequeue, ctx)
	}

	logger.Info("AuthSecret for AzureManagedRedis updated")
