...
func scopeEnsureCommonFields(ctx context.Context, st composed.State) (error, context.Context) {
	state := st.(*State)

	// set name
	state.ObjAsScope().Name = state.gardenerCluster.GetName() // same name as Kyma and GardenerCluster CR
	state.ObjAsScope().Namespace = state.gardenerCluster.GetNamespace()
	...
	for _, label := range cloudcontrolv1beta1.ScopeLabels {
		state.ObjAsScope().Labels[label] = state.gardenerCluster.GetLabels()[label]
	}
