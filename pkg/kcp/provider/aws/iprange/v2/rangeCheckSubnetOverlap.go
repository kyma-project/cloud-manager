...
func rangeCheckSubnetOverlap(ctx context.Context, st composed.State) (error, context.Context) {
	state := st.(*State)
		...
		for _, r := range state.ObjAsIpRange().Status.Ranges {
			...
			if util.CidrOverlap(rangeCidr.CIDR(), subnetCidr.CIDR()) {
				...
				return composed.PatchStatus(state.ObjAsIpRange()).
					...
					SuccessLogMsg("Forgetting KCP IpRange with cidr overlap with existing subnet").
					Run(ctx, state)
			}
		}
	}
