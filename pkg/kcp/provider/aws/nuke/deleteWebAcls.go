...
func deleteWebAcls(ctx context.Context, st composed.State) (error, context.Context) {
	...
	for _, rks := range state.ProviderResources {
		if rks.Kind == "WafPolicy" && rks.Provider == cloudcontrolv1beta1.ProviderAws {
			for _, obj := range rks.Objects {
				resource := obj.(WafPolicyResource)
				...
				if err != nil {
					...
					logger.Error(err, "Error requesting WAF Policy deletion", "id", resource.GetId())
					// Continue anyway (non-fatal error pattern)
				} else {
					logger.Info("Requested WAF Policy deletion", "id", resource.GetId(), "name", name)
				}
			}
		}
	}
