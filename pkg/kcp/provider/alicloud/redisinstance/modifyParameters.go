...
func modifyParameters(ctx context.Context, st composed.State) (error, context.Context) {
	...
	if state.instance.Config != "" {
		...
		if err := json.Unmarshal([]byte(state.instance.Config), &raw); err != nil {
			return composed.LogErrorAndReturn(err,
				"Error parsing AliCloud r-kvstore instance config JSON",
	...
	// Note: after sending "{}", AliCloud may re-populate Config with system-default keys on
	// subsequent DescribeInstance calls. The longer requeue delay limits the rate of retries
	// in that edge case (the "{}" send is idempotent and eventually system defaults stabilise).
	if len(desired) == 0 {
