...
func addUpdatingCondition(ctx context.Context, st composed.State) (error, context.Context) {
	...
	if isChanging {
		return composed.UpdateStatus(kcp).
			SetCondition(metav1.Condition{
				...
				Message: "AliCloud r-kvstore instance is updating.",
			}).
			SuccessErrorNil().
			ErrorLogMessage("Error adding Updating condition to AliCloud RedisInstance").
			Run(ctx, st)
	}
	...
	return composed.UpdateStatus(kcp).
		...
		SuccessErrorNil().
		ErrorLogMessage("Error removing Updating condition from AliCloud RedisInstance").
		Run(ctx, st)
}
