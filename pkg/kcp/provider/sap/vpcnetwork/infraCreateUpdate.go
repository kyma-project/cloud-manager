...
func infraCreateUpdate(ctx context.Context, st composed.State) (error, context.Context) {
	...
	out, err := CreateInfra(ctx, WithName(state.ObjAsVpcNetwork().Status.Identifiers.Name), WithCidrBlocks(state.ObjAsVpcNetwork().Spec.CidrBlocks), WithClient(state.sapClient))
