...
func statusReady(ctx context.Context, st composed.State) (error, context.Context) {
	...
	if state.ObjAsNfsInstance().Status.State != cloudcontrolv1beta1.StateReady {
		state.ObjAsNfsInstance().Status.State = cloudcontrolv1beta1.StateReady
		changed = true
	}
	...
	return composed.PatchStatus(state.ObjAsNfsInstance()).
		...
		ErrorLogMessage("Error patching SAP NfsInstance status after setting ready condition").
		Run(ctx, state)
}
