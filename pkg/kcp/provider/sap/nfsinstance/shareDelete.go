...
func shareDelete(ctx context.Context, st composed.State) (error, context.Context) {
	...
	err := state.sapClient.DeleteShare(ctx, state.share.ID)
	if err != nil && !gophercloud.ResponseCodeIs(err, http.StatusNotFound) {
		...
		return composed.PatchStatus(state.ObjAsNfsInstance()).
			SetExclusiveConditions(metav1.Condition{
				...
				Message: "Error deleting SAP share",
			}).
			FailedError(composed.StopWithRequeueDelay(util.Timing.T60000ms())).
			ErrorLogMessage("Error patching SAP NfsInstance status after error deleting share").
			Run(ctx, state)
	}
	...
	return composed.PatchStatus(state.ObjAsNfsInstance()).
		ErrorLogMessage("Error patching SAP NfsInstance status after share delete").
		SuccessErrorNil().
		Run(ctx, state)
}
