...
func sccServicesEnable(ctx context.Context, st composed.State) (error, context.Context) {
	state := st.(*State)
	logger := composed.LoggerFromCtx(ctx)

	updated := false
	for svcID, svc := range state.sccServices {
		if svc.IntendedEnablementState == securitycentermanagementpb.SecurityCenterService_ENABLED {
			continue
		}
		logger.Info("Enabling SCC service", "serviceId", svcID)
		...
		if err != nil {
			return composed.LogErrorAndReturn(err, "Error enabling SCC service "+svcID, composed.StopWithRequeue, ctx)
		}
		updated = true
	}
