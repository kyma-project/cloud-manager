...
func checkLocalOperation(ctx context.Context, st composed.State) (error, context.Context) {
	...
	op, err := state.client.GetPeeringOperation(ctx, state.LocalNetwork().Spec.Network.Reference.Gcp.GcpProject, state.ObjAsVpcPeering().Status.LocalPeeringOperation)
	if err != nil {
