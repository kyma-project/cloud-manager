...
func checkRemoteOperation(ctx context.Context, st composed.State) (error, context.Context) {
	...
	op, err := state.client.GetPeeringOperation(ctx, state.RemoteNetwork().Spec.Network.Reference.Gcp.GcpProject, state.ObjAsVpcPeering().Status.RemotePeeringOperation)
	if err != nil {
