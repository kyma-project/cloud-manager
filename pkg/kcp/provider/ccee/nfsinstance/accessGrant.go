...
func accessGrant(ctx context.Context, st composed.State) (error, context.Context) {
	...
	logger.Info("Granting CCEE share access")
	...
	if err != nil {
		...
		return composed.PatchStatus(state.ObjAsNfsInstance()).
			...
			ErrorLogMessage("Error patching CCEE NfsInstance status after grant access error").
	...
	return composed.PatchStatus(state.ObjAsNfsInstance()).
		ErrorLogMessage("Error patching CCEE NfsInstance status with accessRightId").
		SuccessErrorNil().
		Run(ctx, state)
}
