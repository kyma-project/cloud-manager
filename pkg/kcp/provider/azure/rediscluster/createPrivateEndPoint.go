...
func createPrivateEndPoint(ctx context.Context, st composed.State) (error, context.Context) {
	...
	privateEndpointClusterName := state.ObjAsRedisCluster().Name
	subnetName := azurecommon.AzureCloudManagerResourceGroupName(state.Scope().Spec.Scope.Azure.VpcNetwork)
	redisClusterResourceId := azureutil.NewRedisInstanceResourceId(
		state.Scope().Spec.Scope.Azure.SubscriptionId,
		resourceGroupName,
		ptr.Deref(state.azureRedisCluster.Name, "")).String()
		...
		armnetwork.PrivateEndpoint{
			...
			Properties: &armnetwork.PrivateEndpointProperties{
				...
				PrivateLinkServiceConnections: []*armnetwork.PrivateLinkServiceConnection{
					{
						Name: to.Ptr(privateEndpointClusterName),
						Properties: &armnetwork.PrivateLinkServiceConnectionProperties{
							PrivateLinkServiceID: to.Ptr(redisClusterResourceId),
	...
	if err != nil {
		...
		if err != nil {
			return composed.LogErrorAndReturn(err,
				"Error updating RedisCluster status due failed azure PrivateEndpoint creation",
