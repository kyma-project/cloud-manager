...
func initRemoteClient(ctx context.Context, st composed.State) (error, context.Context) {
	...
	client, err := state.clientProvider(
		...
		state.remotePrivateDnsZoneId.Subscription,
		remoteTenantId,
		auxiliaryTenants...,
	)
	...
	return composed.PatchStatus(state.ObjAsAzureVNetLink()).
		SetExclusiveConditions(metav1.Condition{
			...
			Message: fmt.Sprintf("Failed creating Azure client for tenant %s subscription %s", state.remotePrivateDnsZoneId.Subscription, remoteTenantId),
		}).
