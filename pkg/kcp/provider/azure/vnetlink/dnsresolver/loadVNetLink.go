...
func loadVNetLink(ctx context.Context, st composed.State) (error, context.Context) {
	...
	vnetLink, err := state.remoteClient.GetDnsResolverVNetLink(ctx,
	...
	return azuremeta.HandleError(err, state.ObjAsAzureVNetLink()).
		...
		WithDefaultMessage("Failed loading DNS resolver VirtualNetworkLink").
		WithTooManyRequestsMessage("Too many requests on loading DNS resolver VirtualNetworkLink").
		WithUpdateStatusMessage("Error updating KCP AzureVNetLink status on failed loading of DNS resolver VirtualNetworkLink").
		WithNotFoundMessage("DNS resolver VirtualNetworkLink not found").
		Run(ctx, state)
}
